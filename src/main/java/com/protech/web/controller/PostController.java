...
import com.protech.util.NewPostFormValidator;
...
public class PostController {
	...
	public String getPost(Model model, @PathVariable Long postId,
		...
		// Added on 25/7/2024
		List<Comment> comments = attributes.get("comments") == null ? null : (List<Comment>)attributes.get("comments");
		Integer cmt_size = comments == null ? 0 : comments.size();
		Integer totalPages = cmt_size / 10 + 1;
	...
	public String processNewPost(@Valid @ModelAttribute("postDto") PostDto postDto, BindingResult bindingResult,
		...
		Post post = this.postService.createNewPost(postDto, session);
		if (null == post) {
	...
	public String processNewComment(@PathVariable Long postId,
		...
		Comment comment = this.commentService.createNewCommentOnPost(postId, commentDto, session);
		if (null == comment) {
